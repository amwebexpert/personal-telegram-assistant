...
# React normalize (orchestration)

This skill applies to **React and TypeScript** projects. It orchestrates two other skills; each is the source of truth for its own workflow — do not duplicate their steps here; read and execute them as written.
...
**Tools:** Use only what you need to read and edit code (Read, Write, edits, Grep, Glob, shell when necessary). Focus on concrete edits.
...
2. **Run react-coding-standards** — Read [../react-coding-standards/SKILL.md](../react-coding-standards/SKILL.md) and the reference files it specifies. Execute that skill's full two-phase workflow (Phase 1: collect violations, Phase 2: apply corrections) on all files in scope. Follow the skill as written; do not paraphrase its workflow.

3. **Run react-single-responsibility** — Read [../react-single-responsibility/SKILL.md](../react-single-responsibility/SKILL.md). Execute its simplification strategies on all files in scope. Let that skill define decomposition order, structure, and rules.
